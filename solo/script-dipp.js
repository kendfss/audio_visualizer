...
function setTitles() {
  if (currentTrack) {
    if (currentTrack.paused) {
      pageTitle.innerHTML = 'visu4liz3r';
      trackNameBox.innerHTML = 'visu4liz3r';
...
function drawRotatingVisualizer(bufferLength, barWidth, byteArray, floatArray) {
  ...
  for (let i = 0; i < bufferLength; i += 1) {
    ...
    // let hue = 270 * Math.round(barHeight / peakByte);
    ctx.fillStyle = hsl(hue, 20, 50);
    // ctx.strokeStyle = hsl(hue, 50 * (barHeight / peakByte), 50);
    ctx.strokeStyle = rgb(0, 0, 0);
    
    // ctx.strokeRect(0, 0, barWidth * 0.7, barHeight * 1.2);
...
// function drawCircle(floatArray) {
  // ctx.fillStyle = sketchpad.style.background;
  // ctx.arc(sketchpad.width/2, sketchpad.height/2, lastRadius, 0, 2 * Math.PI);
  // ctx.fill();
  // let max = maximum(floatArray.map(Math.abs));
  // lastRadius = 13 * (max / peakFloat);
  
  // ctx.fillStyle = hsl(360-(Math.random()*360 * (max / peakByte)), 20, 50);
  // ctx.fillStyle = hsl(360, 20, 100);
  // ctx.arc(sketchpad.width/2, sketchpad.height/2, lastRadius, 0, 2 * Math.PI);
  // ctx.fill();
// }
...
function drawCircle(floatArray) {
  ...
  for (let i = 0; i < fftSize; i ++) {
    let radius = floatArray[i];
    let shift = (2 * Math.PI) / floatArray.length;
    ctx.fillStyle = hsl(360 * (i * floatArray.length), 50, 50);
    ctx.arc(sketchpad.width/2, sketchpad.height/2, radius, angle, angle + shift);
    angle += shift;
  }
}
...
playButton.addEventListener('click', function() {
  ...
  function animate() {
    ...
    drawRotatingVisualizer(bufferLength, xpos, barWidth, byteArray, floatArray);
